...
import 'package:dekiru/models/habitDB.dart';
...
class Habit {
  ...
  Map<String, dynamic> toMap() {
    //preparo l'oggetto per essere salvato
  ...
  Habit.fromMap(Map<String, dynamic> map) {
    //converto rappresentazione testuale della classe in una sua istanza
    id = map['id'];
    name = map['name'];
    if (map['time'] != null) {
  ...
  bool toggelDate(DateTime date) {
    ...
    final int index = data.indexOf(dateString); //cerco in data la mia date
    if (index == -1) {
      //se non c'è aggiungi
      data.add(dateString);
      return true;
