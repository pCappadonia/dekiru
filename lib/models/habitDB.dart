...
class HabitDbProvider {
  ...
  initDB() async {
    ...
    return await openDatabase(
      ...
      onConfigure: (Database db) async {
        await db.execute('PRAGMA foreign_keys = ON'); //abilito foreign keys
      },
    );
  }
  ...
  Future<List<Habit>> getAllHabits() async {
    ...
    results.forEach((result) {
      List<String> data = detail
          .where((element) =>
              element['habit_id'] ==
              result[
                  'id']) //filtro solo gli elementi di detail con habit_id corrispondente a id
          .map<String>((e) => e['date']) //date del mio habit
          .toList();
      Habit habit = Habit.fromMap(result);
      habit.data = data; //date del mio habit
      ...
      if (result['time'] != null) {
        ...
        List<int> day =
            dataReminder['weekday'] //stringa contenente giorni "1,2,3,4,5,6,7"
                .split(',') //"1" "2" "3" "4" "5"
                .map<int>((e) => int.parse(e)) //int x = 1
                .toList(); //lista degli int
    ...
    if (orderState['value'] != '') {
      ...
      habits.sort((a, b) {
        ...
        if (indexA == -1) indexA = orderList.length; //se non esiste è l'ultimo
  ...
  Future<bool> toggleDate(Habit habit, DateTime date) async {
    ...
    if (result == 0) {
      ...
      return false; //se non esisteva lo aggiungi, se esiste lo cancelli
    }
  }
  ...
  Future<Map<String, dynamic>> getOrderState() async {
    ...
    if (result.length == 0) {
      ...
      await db.insert('Option', data); //lo inserisco in option
      return data;
    } else {
      //se esiste habit_order lo ritorna
      return result.first;
    }
  }
